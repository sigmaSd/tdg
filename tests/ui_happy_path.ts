...
export async function runUITests(page: Page) {
  ...
  const day15Initial = page.locator("div.cursor-pointer:not(.bg-gray-50)")
  ...
  const personButton = dropdown.locator("button").filter({
    hasText: new RegExp(`^\\s*${newPersonName}(\\s|$)`),
  });
  ...
  const assignedCell = page.locator("div.cursor-pointer").filter({
    hasText: new RegExp(`^15.*${newPersonName}`),
  });
  ...
  await page.waitForSelector(
    'button:has-text("Generate Schedule"):not([disabled])',
    { timeout: 30000 },
  );
  ...
  assert(
    assignmentsCount > 1,
    "Schedule generation failed: Only 1 assignment found (the manual one)",
  );
