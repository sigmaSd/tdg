...
browserTest(
  "Hierarchical Fairness - Weekend Priority over Workload",
  async () => {
...
browserTest(
  "Hierarchical Fairness - Workload breaks ties in Weekend Rule",
  async () => {
    const settings: Settings = {
      avoidConsecutive: false,
      enableScoring: false,
    ...
    assertEquals(
      ...
      true,
      `Alice should have gotten approx half weekends (got ${
        weekendCounts["1"]
      })`,
    );
    assertEquals(
