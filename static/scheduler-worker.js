(() => {
  ...
  async function solve(z3, data) {
    const { people, year, month, settings, customScores, fixedAssignments } = data;
    ...
    for (let d = 0; d < totalDays; d++) {
      ...
      if (fixedAssignments[dateString]) {
        const personIdx = people.findIndex(
          (p) => p.id === fixedAssignments[dateString]
        );
        if (personIdx !== -1) {
      ...
      if (fixedPersonId) {
        ...
        if (idx !== -1) {
          candidates.push({
            ...
            weekendWorkload: settings.fairWeekend ? weekendCounts[idx][dayOfWeek] || 0 : 0,
        ...
        for (let idx = 0; idx < n; idx++) {
          ...
          if (settings.avoidConsecutive && d > 0 && currentSchedule[formatDate(new Date(year, month, d))] === people[idx].id) continue;
          let futureAvail = 0;
          for (let f = d; f < totalDays; f++) {
            if (!people[idx].unavailable.includes(
          ...
          if (settings.fairWeekend && (dayOfWeek === 0 || dayOfWeek === 6)) {
            for (let f = d; f < totalDays; f++) {
              ...
              if (getDay(fd) === dayOfWeek && !people[idx].unavailable.includes(formatDate(fd))) futureWeekendAvail++;
            }
          }
          candidates.push({
            idx,
  ...
  // static/scheduler-worker.js.entry.tmp.js
